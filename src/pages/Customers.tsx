...
import { 
  ...
  Trash2, Edit2, AlertCircle, FileText, Briefcase, ChevronRight,
  SlidersHorizontal, PlusCircle
} from 'lucide-react';
import { Customer, Agent, CustomFieldDefinition } from '../types';
...
export const Customers = () => {
  ...
  const [customerActiveSubTab, setCustomerActiveSubTab] = useState<'profiles' | 'custom_fields'>('profiles');
  ...
  return (
    <div className="flex flex-col gap-6 w-full pb-10 select-none">
      ...
      <PageBanner 
        ...
        description="Lưu trữ hồ sơ, quản lý tương tác và tùy biến trường thông tin khách hàng linh hoạt."
      />
      ...
      <div className="flex border border-slate-200/60 bg-white rounded-[10px] p-1 shadow-sm w-full max-w-[420px]">
        ...
        <button
          type="button"
          onClick={() => setCustomerActiveSubTab('custom_fields')}
          className={cn(
            "flex-1 flex items-center justify-center gap-2 py-2 rounded-[8px] text-[11.5px] font-bold transition-all cursor-pointer",
            customerActiveSubTab === 'custom_fields'
              ? 'bg-blue-600 text-white shadow-sm'
              : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50'
          )}
        >
          <SlidersHorizontal size={14} />
          <span>Trường dữ liệu mở rộng</span>
        </button>
      </div>

      {/* 3. THẺ THỐNG KÊ (Statistics Cards) */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200 rounded-xl p-4.5 hover:shadow-md transition-all">
          <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Tổng số khách hàng</p>
          <h3 className="text-xl font-black text-slate-800 mt-1">{totalCount}</h3>
          <p className="text-[10px] text-slate-500 font-semibold mt-1">Lưu trữ trên đám mây</p>
        </div>

        <div className="bg-white border border-slate-200 rounded-xl p-4.5 hover:shadow-md transition-all">
          <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Khách cá nhân</p>
          <h3 className="text-xl font-black text-slate-800 mt-1">{individualCount}</h3>
          <p className="text-[10px] text-slate-500 font-semibold mt-1">Cá nhân &amp; gia đình</p>
        </div>

        <div className="bg-white border border-slate-200 rounded-xl p-4.5 hover:shadow-md transition-all">
          <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Doanh nghiệp / Tổ chức</p>
          <h3 className="text-xl font-black text-slate-800 mt-1">{companyCount}</h3>
          <p className="text-[10px] text-slate-500 font-semibold mt-1">Đối tác doanh nghiệp</p>
        </div>

        <div className="bg-white border border-slate-200 rounded-xl p-4.5 hover:shadow-md transition-all">
          <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Khách hàng VIP</p>
          <h3 className="text-xl font-black text-slate-800 mt-1">{vipGoldPlatinum}</h3>
          <p className="text-[10px] text-slate-500 font-semibold mt-1">Hạng Vàng &amp; Platinum</p>
        </div>
      </div>
      ...
      {customerActiveSubTab === 'profiles' ? (
        <div className="bg-white rounded-[12px] border border-slate-200/80 shadow-md p-6 flex flex-col gap-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
            <div className="flex items-center gap-3">
              <h2 className="text-base font-extrabold text-slate-800">Thông tin cơ sở dữ liệu khách hàng</h2>
              <div className="bg-slate-100/80 p-1 rounded-[10px] flex items-center gap-1 border border-slate-200/50 shadow-inner shrink-0">
                <button
                  ...
                  className={cn(
                    ...
                    customerViewMode === 'table' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800'
                  )}
                >
                  Dạng bảng
                </button>
                <button
                  ...
                  className={cn(
                    "px-3 py-1 rounded-[8px] text-[10px] font-bold transition-all flex items-center gap-1.5 cursor-pointer",
                    customerViewMode === 'cards' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800'
                  )}
                >
                  Dạng thẻ
                </button>
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className="bg-blue-600 hover:bg-blue-700 text-white px-3.5 py-1.5 rounded-[8px] text-[10px] font-bold uppercase tracking-wider transition-all flex items-center gap-1.5 cursor-pointer"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
            ...
            <div className="flex items-center gap-2.5">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 shrink-0">LOẠI:</span>
              <select 
            ...
            {/* VIP filter dropdown */}
            <div className="flex items-center gap-2.5">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 shrink-0">PHÂN HẠNG:</span>
              <select 
          ...
          <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-sm flex flex-col">
            <div className="overflow-x-auto flex-1">
              <table className="w-full text-left text-xs whitespace-nowrap">
                <thead className="bg-slate-50 text-slate-500 font-bold border-b border-slate-200/80 sticky top-0 z-10">
                  <tr>
        ...
        <div className="bg-white rounded-[12px] border border-slate-200/80 shadow-md p-6 flex flex-col gap-6">
          <div className="flex justify-between items-center pb-4 border-b border-slate-100">
            <div>
              <h3 className="text-sm font-extrabold text-slate-800">Cấu hình trường mở rộng</h3>
              <p className="text-[11px] text-slate-500 font-semibold mt-0.5">Thêm các trường dữ liệu tùy chỉnh cho biểu mẫu nhập liệu khách hàng.</p>
            </div>
            {!isCreatingField && (
              <button
                type="button"
                ...
                className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-[8px] text-[10px] font-bold uppercase tracking-wider cursor-pointer transition-colors"
              >
                + Thêm trường mới
              </button>
            )}
          ...
          {isCreatingField ? (
            <form onSubmit={handleCreateCustomField} className="space-y-4 max-w-md bg-slate-50 p-5 rounded-xl border border-slate-200">
              <div>
                <label className="text-[10px] font-bold text-slate-500 block mb-1 uppercase tracking-wider">Tên trường dữ liệu *</label>
                <input
                  type="text"
                  required
                  placeholder="Ví dụ: Link Facebook Profile, Mã số thuế..."
                  value={newFieldName}
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-500 block mb-1 uppercase tracking-wider">Loại dữ liệu</label>
                <select
                  value={newFieldType}
                  onChange={(e) => setNewFieldType(e.target.value as any)}
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 font-semibold cursor-pointer"
                >
              ...
              {newFieldType === 'select' && (
                <div>
                  <label className="text-[10px] font-bold text-slate-500 block mb-1 uppercase tracking-wider">Tùy chọn (Phân cách bởi dấu phẩy)</label>
                  <input
                    type="text"
                    required
                    placeholder="Ví dụ: Lựa chọn A, Lựa chọn B, Lựa chọn C"
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                <label htmlFor="newFieldRequired" className="text-[11px] font-bold text-slate-600 cursor-pointer select-none">
                  Bắt buộc nhập khi khai báo thông tin khách hàng
                </label>
              </div>
              ...
              <div className="flex justify-end gap-2 pt-4">
                <button
                  ...
                  className="px-3 py-1.5 text-slate-500 hover:bg-slate-100 rounded-lg text-xs font-bold transition-colors cursor-pointer"
                >
                  Hủy bỏ
                </button>
                <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {customFields.map(field => (
                <div key={field.id} className="border border-slate-200 p-5 rounded-xl flex flex-col justify-between hover:border-blue-300 transition-all hover:shadow-md bg-slate-50/20">
                  <div>
                    <h4 className="text-xs font-extrabold text-slate-800">{field.name}</h4>
                    <span className="text-[9px] bg-slate-100 text-slate-500 border border-slate-200 font-extrabold px-2 py-0.5 rounded uppercase mt-2 inline-block">
                      Kiểu: {field.type}
                    </span>
                    {field.isRequired && (
                      <span className="text-[9px] bg-red-50 text-red-600 border border-red-100 font-extrabold px-2 py-0.5 rounded uppercase mt-2 ml-1.5 inline-block">
                        Bắt buộc
                      </span>
                    )}
                  </div>
                  <button
                    ...
                    className="text-slate-400 hover:text-red-600 font-extrabold text-[10px] mt-4 self-end flex items-center gap-1 cursor-pointer transition-colors"
                  >
                    <Trash2 size={12} /> Xóa trường dữ liệu này
                  </button>
                </div>
              ))}
            </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => setFormState(p => ({ ...p, gender: e.target.value as any }))}
                          className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 font-semibold"
                        >
